...
- Event system in React allows you to pass functions when handling events.
- if you see a `?` in the URL, it means the browser is sending a GET request by default, 
causing a full page refresh. This is why you use `preventDefault()`
- SyntheticEvent: Object created by React that mimics browser events. You don't realize that
you're interacting with this. Does this for performance reasons
- `console.dir` with provide just ReactDOM node with properties if you want to clean up
console logging a bit.
- `htmlFor` is important for form organization and labeling html elements that are for the
  same input in pure html, it's just `for` but for is a reserved word in React, so you 
  have to use `htmlFor` for labeling
   - Also will now focus on input when you use `htmlFor`


❓🤔: Getting `[Violation] 'submit' handler took 2110ms` ? Why?

❓🤔: Skipped the elements in `event.target` and went straight to username. Is this bad
practice? Why.


#### 💫💫💫💫💫 Extra Credit 1 💫💫💫💫💫💫
- A `ref` is an object that stays consistent between renders of React component
   - `current` property that can be updated to any value at any time, so you can pass 
     a ref to a React element and it will update the `current` property 
  - `ref` is a React Hook
- preference should be for the solution, but there will be situations in the future
  where you would want to use refs.
  
  ❓🤔: Is there a situation in MC where refs are used?


#### 💫💫💫💫💫 Extra Credit 2 💫💫💫💫💫💫
- There isn't a default behavior on change events
- `const [state, mechanismToUpdateState] = React.setState(startingState)`

#### 💫💫💫💫💫 Extra Credit 3 💫💫💫💫💫💫
- Controlled input: can define value instead of calling `target.element.value`
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=ashley.mains%40mailchimp.com).
