...
#### 💫💫💫💫💫 Solution 💫💫💫💫💫💫
- Why the key prop is necessary for rendering arrays: React doesn't know by default
what is changing in a rendered array, even with `htmlFor`. You HAVE to provide the key
  in the key-value pair as an `item.id in a `li` tag
- `index` of a component silences the warning, but doesn't remove the problem, so you
HAVE to use a key-value pair

❓🤔: How would you describe how `.find` and `.filter` work in javascript? What's the
pseudocode template?
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=07%3A%20Rendering%20Arrays&em=ashley.mains%40mailchimp.com).
